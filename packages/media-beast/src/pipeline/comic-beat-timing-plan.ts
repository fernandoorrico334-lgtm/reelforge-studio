...
export type ComicBeatTimingScenePlan = {
  ...
  durationSeconds: number;
  cutStyle: "hard_cut" | "flash_cut" | "whoosh_cut" | "hold_reveal";
...
function cutStyleFor(beat: ComicArcScriptBeat, visual?: ComicArcVisualDirection): ComicBeatTimingScenePlan["cutStyle"] {
  if (beat.role === "hook" || beat.role === "climax") return "flash_cut";
  if (beat.role === "tension" || visual?.primaryTarget === "duo_conflict") return "whoosh_cut";
...
function scoreScene(input: { beat: ComicArcScriptBeat; visual?: ComicArcVisualDirection | undefined; durationSeconds: number; events: ComicBeatTimingEvent[] }) {
  let score = 66;
  if (input.durationSeconds >= 5 && input.durationSeconds <= 10) score += 8;
  ...
  const scenes = input.beats.map((beat, index) => {
    ...
    const events: ComicBeatTimingEvent[] = [
      ...
      },
      {
        ...
        instruction: `${visual?.cameraMove ?? "context_slow_push"}; anchor=${visual?.anchorPoint.x ?? 0.5},${visual?.anchorPoint.y ?? 0.5}; scale=${visual?.startScale ?? 1.02}->${visual?.endScale ?? 1.1}.`
      },
      {
        type: "caption_out",
    ...
    if (beat.role === "hook" || beat.role === "climax" || visual?.primaryTarget === "impact_zone") {
      events.push({
        ...
        intensity: 10,
        instruction: "Punch zoom curto no frame de impacto; usar shake leve e flash se for climax/hook."
      });
      events.push({
        type: "sfx_hit",
    ...
    if (durationSeconds < 5) warnings.push("scene_too_short_for_readable_comic_panel");
